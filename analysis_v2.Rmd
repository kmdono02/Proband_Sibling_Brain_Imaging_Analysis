...
Now, we conduct the same linear regression analysis as was done before but also include a set of possible confouding variables for eahch model.  The variables are: study site for infant, age of the infant at MRI scan, sex of infant, and proband IQ (VABS ABC score).  We print the same output as in the previous linear regression analysis.
...
for(i in 1:length(brain_vars)){
   for(j in 1:length(time_list)){
    frmla_val <-  
      as.formula(paste0(brain_vars[i],
             "~RiskGroup+SCQ_Pro_Score+RiskGroup*SCQ_Pro_Score+
                     Site+AgeMRI+Cand_Sex+VABS_Pro_ABC_SS+VABS_Pro_ABC_SS*RiskGroup"))
...
for(i in 1:length(brain_vars)){
  frmla_val <-  
      as.formula(paste0(brain_vars[i],
             "~Pro_SCQgrp + Cand_Sex + AgeMRI + Site + VABS_Pro_ABC_SS"))
   for(j in 1:length(time_list)){
    formula = frmla_val
...
for(i in 1:length(brain_vars)){
  frmla_val <-  
      as.formula(paste0(brain_vars[i],
             ...
             Site+Cand_Sex+VABS_Pro_ABC_SS+VABS_Pro_ABC_SS*RiskGroup"))
  ...
  sort_vec <- 
    c("(Intercept)", "SCQ_Pro_Score", "VABS_Pro_ABC_SS", "Cand_SexMale", 
      "RiskGroupHR-ASD",
      "RiskGroupHR-ASD:SCQ_Pro_Score",  "RiskGroupHR-ASD:AgeMRI", 
      "RiskGroupHR-ASD:VABS_Pro_ABC_SS",
...
for(i in 1:length(brain_vars)){
  ...
  sort_vec <- c("(Intercept)", "SCQ_Pro_Score", "VABS_Pro_ABC_SS",
...
med_model_fit_tsa_scq <-
  lm(formula=TSA~SCQ_Pro_Score+
     ...
     data=summ_stats_data_long_12m+VABS_Pro_ABC_SS+VABS_Pro_ABC_SS*RiskGroup)
...
eye_gaze_long_full <-
  eye_gaze_long_full %>%
  mutate(V00_V06_ind = ifelse(0<=AgeMRI & AgeMRI<6, 1,
                              ifelse(is.na(AgeMRI)==1, NA, 0)),
         V06_V12_ind = ifelse(6<=AgeMRI & AgeMRI<12, 1,
                              ifelse(is.na(AgeMRI)==1, NA, 0)),
         V12_V24_ind = ifelse(12<=AgeMRI & AgeMRI<24, 1,
                              ifelse(is.na(AgeMRI)==1, NA, 0)),
         V24_plus_ind = ifelse(24<=AgeMRI, 1,
                              ifelse(is.na(AgeMRI)==1, NA, 0)))
